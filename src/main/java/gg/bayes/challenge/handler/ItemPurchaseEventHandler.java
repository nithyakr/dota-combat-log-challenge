...
@Slf4j
@Component
public class ItemPurchaseEventHandler extends AbstractEventHandler {

    //npc_dota_([a-zA-Z]+(_[a-zA-Z]+)+) if needed to include non-hero purchases
    @Value("${bayes.dota.item-purchased-event-format}")
    private String itemPurchasedEventPattern;
    private Pattern pattern;

    @Override
    public void afterPropertiesSet() {
        pattern = Pattern.compile(itemPurchasedEventPattern);
    }
