...
public class MatchController {
    ...
    public MatchController(LogFileService logFileService,
    ...
    public ResponseEntity<Long> ingestCombatLog(@RequestBody @NotBlank String combatLog) {
        MatchEntity savedEntity = logFileService.processFile(combatLog);
    ...
    public ResponseEntity<List<HeroKills>> getMatch(@PathVariable("matchId") Long matchId) {
        List<HeroKills> heroKills = matchService.listHeroKillsForMatch(matchId);
        log.debug("Found [{}] hero kills in the match [{}]", heroKills.size(), matchId);
    ...
    public ResponseEntity<List<HeroItem>> getHeroItems(
        ...
        List<HeroItem> heroItems = matchService.listItemsForHero(matchId, heroName);
        log.debug("Found [{}] Purchases by Hero [{}] in the match [{}]", heroItems.size(), heroName, matchId);
    ...
    public ResponseEntity<List<HeroSpells>> getHeroSpells(
            @PathVariable("matchId") Long matchId,
            @PathVariable("heroName") String heroName) {
        List<HeroSpells> heroSpells = matchService.listNumberOfSpellCasts(matchId, heroName);
        log.debug("Found [{}] hero spells by [{}] in the match [{}]", heroSpells.size(), heroName, matchId);
    ...
    public ResponseEntity<List<HeroDamage>> getHeroDamages(
            @PathVariable("matchId") Long matchId,
            @PathVariable("heroName") String heroName) {
        List<HeroDamage> heroDamages = matchService.calculateDamageDoneByHero(matchId, heroName);
        log.debug("Found [{}] Damage items by Hero [{}] in the match [{}]", heroDamages.size(), heroName, matchId);
